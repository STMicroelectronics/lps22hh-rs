...
fn main() -> ! {
    ...
    let _ = sensor.fifo_watermark_set(32);
    let _ = sensor.fifo_stop_on_wtm_set(1);
    let _ = sensor.fifo_mode_set(FMode::StreamMode);

    let _ = sensor.int_notification_set(Lir::Latched);
    ...
    let _ = sensor.data_rate_set(Odr::_10hz);
    ...
    loop {
        ...
        let wtm_flag = sensor.fifo_wtm_flag_get().unwrap();

        if wtm_flag == 1 {
            let num = sensor.fifo_data_level_get().unwrap();
            writeln!(tx, "-- FIFO interrupt {}", num).unwrap();
            for _i in 0..num {
                let press_raw = sensor.fifo_pressure_raw_get();
                match press_raw {
                    Ok(value) => {
                        writeln!(tx, "press: {} hPa", lps22hh_rs::from_lsb_to_hpa(value))
                            .unwrap()
                    }
                    Err(e) => writeln!(tx, "Error in reading pressure: {:?}", e).unwrap(),
                }

                let temp_raw = sensor.fifo_temperature_raw_get();
                match temp_raw {
                    Ok(value) => writeln!(
                        tx,
                        "temp: {:.2} C",
